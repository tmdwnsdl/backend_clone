...
import com.example.firebasechat.databinding.ItemChatroomsBinding
...
class RecyclerChatRoomAdapter(val context: Context) :
    RecyclerView.Adapter<RecyclerChatRoomAdapter.ViewHolder>() {
    var chatRooms: ArrayList<ChatRoomData> = arrayListOf()   //채팅방 목록
    ...
    fun setupAllUserList() {     //전체 채팅방 목록 초기화 및 업데이트
        FirebaseDatabase.getInstance().getReference("ChatRoom").child("chatRooms")
            .orderByChild("users").equalTo(true)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding =
            ItemChatroomsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        FirebaseDatabase.getInstance().getReference("User").child("users")
            .orderByChild("uid")   //상대방 사용자 키를 포함하는 채팅방 불러오기
            .equalTo(opponent)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onCancelled(error: DatabaseError) {}
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (data in snapshot.children) {
                        ...
                        holder.txt_name.text =
                            data.getValue<User>()!!.name.toString()     //상대방 이름 초괴화
                    }
                }
            })
    ...
    fun setupLastMessageAndDate(holder: ViewHolder, position: Int) { //마지막 메시지 및 시각 초기화
        var lastMessage =
            chatRooms[position].messages!!.values.sortedWith(compareBy({ it.send_time }))    //메시지 목록에서 시각을 비교하여 가장 마지막 메시지  가져오기
                .last()
        holder.txt_message.text = lastMessage.msg                 //마지막 메시지 표시
        holder.txt_date.text = getLastMessageTimeString(lastMessage.send_time)   //마지막으로 전송된 시각 표시
    }
    ...
    fun setupMessageCount(holder: ViewHolder, position: Int) {            //확인되지 않은 메시지 개수 표시
        var unconfirmedCount =
            chatRooms[position].messages!!.filter {
                !it.value.confirmed && !it.value.username.equals(               //메시지 중 확인되지 않은 메시지 개수 가져오기
                    myUid
                )
            }.size
    ...
    class ViewHolder(val binding:ItemChatroomsBinding):
        RecyclerView.ViewHolder(binding.root) {
        var opponentUser = User("", "")
        var chatRoomKey = ""
        var background = binding.bgchatroom
        var txt_name = binding.txtname
        var txt_message = binding.txtchat
        var txt_date = binding.txttime
        var txt_chatCount = binding.txtcount
    }
}

private fun Intent.putExtra(s: String, opponentUser: User) {
    TODO("Not yet implemented")
}

private fun Intent.putExtra(s: String, get: ChatRoomData) {
    TODO("Not yet implemented")
}
